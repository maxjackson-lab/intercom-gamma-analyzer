...
function selectTimePeriod(period) {
    ...
    wizardState.timePeriod = period;
    document.getElementById('step2').style.display = 'none';
    document.getElementById('step3').style.display = 'block';
}

function confirmCustomDates() {
    wizardState.startDate = document.getElementById('startDate').value;
    wizardState.endDate = document.getElementById('endDate').value;
    wizardState.timePeriod = 'custom';
    document.getElementById('step2').style.display = 'none';
    document.getElementById('step3').style.display = 'block';
}
...
function showSummary() {
    ...
    const timeLabels = {
        ...
        'week': '📅 Last Week',
        'month': '📅 Last Month',
        'custom': `📅 ${wizardState.startDate} to ${wizardState.endDate}`
    };
    
    const sourceLabels = {
        'intercom': '💬 Intercom Only',
        'canny': '✨ Canny Only',
        'both': '🔗 Both Sources'
    };
    ...
    document.getElementById('summaryAnalysis').textContent = analysisLabels[wizardState.analysis];
    document.getElementById('summaryTime').textContent = timeLabels[wizardState.timePeriod];
    document.getElementById('summarySource').textContent = sourceLabels[wizardState.source];
    document.getElementById('summaryFormat').textContent = formatLabels[wizardState.format];
    
    document.getElementById('stepConfirm').style.display = 'block';
}

function resetForm() {
    wizardState = {analysis: null, timePeriod: null, source: null, format: null};
    document.querySelectorAll('.form-step').forEach(step => step.style.display = 'none');
    document.getElementById('step1').style.display = 'block';
}

async function executeAnalysis() {
    // Hide confirmation, show running
    document.getElementById('stepConfirm').style.display = 'none';
    document.getElementById('stepRunning').style.display = 'block';
    
    // Build command
    let command = wizardState.analysis + '-analysis';
    
    // Add time period or dates
    if (wizardState.timePeriod === 'custom') {
        command += ` --start-date ${wizardState.startDate} --end-date ${wizardState.endDate}`;
    } else {
        command += ` --time-period ${wizardState.timePeriod}`;
    }
    
    // Add source flags
    if (wizardState.source === 'canny' || wizardState.source === 'both') {
        command += ' --include-canny';
    }
    
    // Add format
    if (wizardState.format === 'gamma') {
        command += ' --generate-gamma';
    }
    
    // Add multi-agent for voice-of-customer
    if (wizardState.analysis === 'voice-of-customer') {
        command += ' --multi-agent --analysis-type topic-based';
    }
    
    // Execute via the existing execution system
