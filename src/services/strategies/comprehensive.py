"""
Comprehensive orchestration strategy for consolidated analyses.
"""
...
class ComprehensiveStrategy(OrchestrationStrategy):
    """
    ...
    def __init__(self):
        ...
        self.logger.info("ComprehensiveStrategy initialized")
    ...
    async def execute(self, context: AgentContext, **kwargs) -> AgentResult:
        """
        Execute comprehensive analysis and return an AgentResult payload.
        """
        ...
        return AgentResult(
            ...
            error_message=None if success else results.get('error') if isinstance(results, dict) else "Unknown error",
            execution_time=time.time() - start_time
        )

    def _derive_confidence(self, results: Dict[str, Any], success: bool) -> float:
        """
        Derive confidence score using validation metadata when available.
        """
        if not success:
            return 0.0
        ...
        if isinstance(validation, dict):
            data_quality = validation.get('data_quality_score')
            if isinstance(data_quality, (int, float)):
                return max(0.0, min(1.0, float(data_quality)))
