...
class StoryDrivenStrategy(OrchestrationStrategy):
    """
    ...
    def __init__(self):
        ...
        self.logger.info("StoryDrivenStrategy initialized")

    def get_strategy_name(self) -> str:
        return "StoryDrivenStrategy"
    ...
    async def execute(
        ...
        **kwargs
        ...
        canny_posts = metadata.get('canny_posts', [])

        start_time = datetime.now()
        results = await self.run_story_driven_analysis(
        ...
        success = isinstance(results, dict) and 'error' not in results
        confidence = self._derive_confidence(results, success)
        ...
        return AgentResult(
            ...
            error_message=None if success else results.get('error') if isinstance(results, dict) else "Story-driven analysis failed",
...
Write for an executive audience - clear, actionable, and focused on business impact."""
    ...
    def _derive_confidence(self, results: Dict[str, Any], success: bool) -> float:
        """Estimate confidence using insight confidence metadata when available."""
        if not success or not isinstance(results, dict):
            return 0.0

        actionable = results.get('actionable_insights', {})
        if isinstance(actionable, dict):
            insight_confidence = actionable.get('insight_confidence', {})
            if isinstance(insight_confidence, dict):
                overall = insight_confidence.get('overall_confidence')
